...
struct Gaussian::GaussianSampling::encapsulation {
	...
	std::unique_ptr<std::mt19937_64> randomGen_t;
...
int64_t Gaussian::GaussianSampling::encapsulation::sampleZ(const double_t sigma, const double_t center) const noexcept {
	...
	// sample
	return (int64_t) round(normalDist(*randomGen));
}
...
inline void Gaussian::GaussianSampling::encapsulation::G_sampleD(int32_t * output, const double_t sigmaa, const double_t * c) const noexcept {
	...
	const int32_t c_dz = (int32_t) floor(c_d); //floor返回不大于 c_d 的最大整数值
...
void Gaussian::GaussianSampling::G_sampleGsMonoThread(int32_t * output, const uint64_t * coset, const uint32_t length) const noexcept {
   ...
   // std::cout<<"G_sampleGsMonoThread"<<std::endl;
...
impl(new encapsulation {.dimension = n, .param = k}) {
	...
	impl->randomGen_t.reset(new std::mt19937_64(rd()));
	impl->randomGen = impl->randomGen_t.get();
...
void Gaussian::GaussianSampling::sampleGPoly(Poly_t * polys, const uint64_t * coset) const noexcept {
    ...
    //std::cout << "thread_share   : "<<thread_share<< std::endl; 
	
    std::fill_n(thread_shares, THREADS_NUMBER, thread_share);//从当前thread_shares开始，将之后的THREADS_NUMBER个元素赋值为thread_share。
    
  /*for(int i=0;i<THREADS_NUMBER;i++){
        std::cout << " thread_shares["<<i<<"]: "<<thread_shares[i]<< std::endl; 
        
    }
	*/
    
	for(uint32_t i = 0; i < n - thread_share*THREADS_NUMBER; ++i) {//n-thread_share*THREADS_NUMBER=n-n=0//why
		++thread_shares[i];
	}
	...
	for(uint32_t i = 0; i < k; ++i) {
     ...
     //for(int i=0;i<n;i++){
       // std::cout << " : samplingzTInd   : "<<samplingzTInd[i]<< std::endl; 
   ...
   // }
	...
	auto lambda = [n](Poly_t * polyz, const NFL_POLY_COEF_TYPE * coefs, const uint32_t length) {
		for(uint32_t i = 0; i < length; ++i) {
           ...
           // std::cout << i<<" : set( "<<*coefs<<" , "<<*coefs+n<<" )"<< std::endl; 
            //std::cout << i<<" : z    : "<<polyz[i]<< std::endl; 
			polyz[i].ntt_pow_phi();
            //std::cout << i<<" : ntt-z: "<<polyz[i]<< std::endl; 
			coefs += n;
		}
	};
