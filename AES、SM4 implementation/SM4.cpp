...
static void _SM4_do(uint8_t* in, uint8_t* out, SM4_Key sm4_key, uint8_t enc);
...
void SM4_Encrypt(uint8_t* plaintext, uint8_t* ciphertext, SM4_Key sm4_key) {
    _SM4_do(plaintext, ciphertext, sm4_key, 0);
}

void SM4_Decrypt(uint8_t* ciphertext, uint8_t* plaintext, SM4_Key sm4_key) {
    _SM4_do(ciphertext, plaintext, sm4_key, 1);
}
...
static void _SM4_do(uint8_t* in, uint8_t* out, SM4_Key sm4_key, uint8_t enc) {
    ...
    for (int i = 0; i < 4; i++) {
        int j = 4 * i;
        x[i] = (in[j + 0] << 24) | (in[j + 1] << 16) | (in[j + 2] << 8) |
            (in[j + 3]);
    }
    // 32轮循环
    for (int i = 0; i < 32; i++) {
        tmp = (enc == 0) ? sm4_key[i] : sm4_key[31 - i];
        tmp = x[1] ^ x[2] ^ x[3] ^ tmp;
        //查找表（S盒+线性变换）
        tmp1 = x[0];
        tmp1 ^= Table0[(tmp >> 24) & 0xFF];
        tmp1 ^= Table1[(tmp >> 16) & 0xFF];
        tmp1 ^= Table2[(tmp >> 8) & 0xFF];
        tmp1 ^= Table3[(tmp >> 0) & 0xFF];
...
int main() {
    ...
    std::cout << "SM4加密100000次时间为： " << (std::chrono::duration <double, std::milli>(running_time).count())  << " ms" << std::endl;
    system("pause");
    return 0;
}